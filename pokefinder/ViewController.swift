...
class ViewController: UIViewController, MKMapViewDelegate, CLLocationManagerDelegate {
    ...
    func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
        ...
        } else { // If the annotation fails, provide a default to avoid nil
            let av = MKAnnotationView(annotation: annotation, reuseIdentifier: annoIdentifier)
            av.rightCalloutAccessoryView = UIButton(type: .detailDisclosure)
            annotationView = av
        }
        
        // Customize the annotation. Ensure it's not nil, and that it can be successfully cast as a PokeAnnotation.
    ...
    // Update the map with Pokemon
    func mapView(_ mapView: MKMapView, regionWillChangeAnimated animated: Bool) {
        let loc = CLLocation(latitude: mapView.centerCoordinate.latitude, longitude: mapView.centerCoordinate.longitude)
        showSightingsOnMap(location: loc)
    }
    
    // Show on AppleMaps when a Pokemon is tapped, the callout appears, and user presses the button 
